...
The CMS uses simple auto-discovery from the local `/admin/config.yml` file:

1. `GIT_REPO_DIRECTORY=.` - Restricts decap-server to current app directory only
2. Standard Decap CMS loading - Auto-discovers config from the same `/admin/` folder
3. No manual initialization - Avoids complexity that can cause cross-app config discovery

> **Important:** Do NOT use manual CMS initialization with fallbacks. See "Manual CMS Initialization Pitfall" section below.
...
## Manual CMS Initialization Pitfall

**Do NOT use manual CMS initialization with fallbacks in a monorepo.**

### The Problem

This pattern seems logical but causes issues:
...
fetch('/admin/config.yml')
  ...
  .then(configText => {
    CMS.init({ config: parseYaml(configText) });
  })
  .catch(() => {
    CMS.init();  // ← PROBLEM: fallback triggers auto-discovery
  });
```

### Why It Fails

When the fetch fails (timing issues, server not ready, network problems), the fallback `CMS.init()` runs **without an explicit config**. This triggers Decap CMS auto-discovery, which can find config files from multiple apps in the monorepo (`docs` and `team-docs`), causing:

```
Error: 'collections' collections names must be unique
```

### Why It Works on Some Machines

On faster machines or with favorable timing, the fetch succeeds and the explicit config is used. On other machines, the fetch may fail, triggering the problematic fallback.

### The Correct Approach

Let Decap CMS use standard auto-discovery from the local `/admin/` folder:

```html
<!-- ✅ DO THIS - Simple and reliable -->
<script src="https://unpkg.com/decap-cms@3.3.3/dist/decap-cms.js"></script>
<script>
  // Register preview styles and templates
  CMS.registerPreviewStyle('...');
  CMS.registerPreviewTemplate('...', ...);
  // No CMS.init() needed - auto-discovers /admin/config.yml
</script>
```

This approach:
- Automatically finds `/admin/config.yml` in the same folder
- Works consistently across all machines
- Doesn't require external YAML parsing libraries
- Has no fallback path that could discover wrong configs
