...
interface ComponentFrontmatter {
  ...
  // Interactive story for code usage section (shows controls panel)
  interactive_storyId?: string;
...
export default function ComponentPage({ children, frontMatter }: ComponentPageProps): JSX.Element {
  ...
  const renderWebCodeUsage = (code: string) => (
    <>
      ...
      {fm.interactive_storyId && (
        <div className={styles.interactivePlayground}>
          <h3>Interactive playground</h3>
          <p>Try different property combinations below:</p>
          <StoryEmbed
            storyId={fm.interactive_storyId}
            interactive
            fixedHeight={700}
          />
        </div>
