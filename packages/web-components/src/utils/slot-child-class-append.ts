...
//  = [ a selector, attribute value,  shallow application option]
...
// Will itterate though the array of slotTargets, applying the attribute value [1] asa class, 
// to all matching selectors [0]; if the slotTarget also contains a 3rd item (shallow application)
// the function will NOT apply the classes to any decendents
// If an optional class string is provided: if the slot is passed more than one element, or 
...
export function iterateAddAttrs(targetingArray:SlotTarget[], shadow:ShadowRoot, theClass:string|undefined|null) : null{
    targetingArray.forEach( (slot:SlotTarget)=>{
        ...
        if (theClass) {
            ...
            if (childCount >= 1){
                ...
                if (firstTag !== "DIV" || childCount > 1) { wrapSlotElements(theSlotEl,theClass); }
                if (firstTag  === "DIV" && childCount == 1) { firstEl.classList.add(theClass); }
            } 
        }  
...
// and coreesponding slot.name  to it, 
