...
describe('va-service-list-item', () => {
  ...
  it('renders with all elements and the critical information component when an action prop is provided', async () => {
    ...
    expect(shadowInnerHTML.replace(/\s+/g, ' ')).toEqualHtml(`
      ...
      <va-critical-information class="hydrated"></va-critical-information>
        <div class="status">
  ...
  it('does NOT render the critical information component when action prop is not passed', async () => {
    ...
    expect(shadowInnerHTML).not.toContain('<va-critical-information');
  });
