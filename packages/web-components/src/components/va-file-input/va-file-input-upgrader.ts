...
const createVisibleInstructions = fileInputEl => {
  ...
  instructions.innerHTML = Sanitizer.escapeHTML(
    `<span class="${DRAG_TEXT_CLASS}">${dragText}</span> <span class="${CHOOSE_CLASS}">${chooseText}</span>`,
  );
...
const addPreviewHeading = (fileInputEl, fileNames) => {
  ...
  if (fileNames.length === 1) {
    previewHeadingText = Sanitizer.escapeHTML(
      `Selected file <span class="usa-file-input__choose">${changeItemText}</span>`,
    );
  } else if (fileNames.length > 1) {
    changeItemText = 'Change files';
    previewHeadingText = Sanitizer.escapeHTML(
      `${fileNames.length} files selected <span class="usa-file-input__choose">${changeItemText}</span>`,
    );
  }
