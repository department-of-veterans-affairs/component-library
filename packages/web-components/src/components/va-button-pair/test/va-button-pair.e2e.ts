...
describe('va-button-pair', () => {
  it('renders', async () => {
    ...
    await page.setContent('<va-button-pair uswds="false"></va-button-pair>');
  ...
  it('passes an axe check', async () => {
    const page = await newE2EPage();
    await page.setContent('<va-button-pair continue uswds="false"></va-button-pair>');
    await axeCheck(page);
  });

  it('fires analytics event when primary button is clicked', async () => {
    const page = await newE2EPage();
    await page.setContent('<va-button-pair confirm uswds="false"></va-button-pair>');
  ...
  it(`doesn't fire analytics event when disableAnalytics is true`, async () => {
    ...
    await page.setContent(
      '<va-button-pair disable-analytics uswds="false"></va-button-pair>',
    );
    const analyticsSpy = await page.spyOnEvent('component-library-analytics');
    const buttons = await page.findAll('va-button-pair >>> va-button');
    await buttons[1].click();
