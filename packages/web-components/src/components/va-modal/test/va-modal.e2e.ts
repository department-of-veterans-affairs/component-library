...
describe('va-modal', () => {
  it('renders', async () => {
    ...
    await page.setContent(`
      <va-modal headline="Example Title" visible>
        <p>
    ...
    expect(element).toEqualHtml(`
      <va-modal aria-label="va-modal-title" aria-modal="true" class="hydrated va-modal" headline="Example Title" role="dialog" visible>
        <mock:shadow-root>
  ...
  it('closeEvent is not triggered using the Escape key when modal is not visible', async () => {
    const page = await newE2EPage();
    await page.setContent(`
      <va-modal headline="Example Title">
        <p>
          A modal may pass any React nodes as children to be displayed within it.
        </p>
      </va-modal>
