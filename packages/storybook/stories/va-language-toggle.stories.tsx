...
export default {
  ...
  parameters: {
    ...
    docs: {
      page: () => (
        <StoryDocs storyDefault={Default} data={languageToggleDocs} />
      ),
    },
  },
};
...
const WithRouterLinksTemplate = ({}) => {
  ...
  return (
    <Fragment>
      <div>
        This example illustrates how to use the component with a router. When{' '}
        <code>router-links</code> is set to <code>true</code>, clicking on a
        link will not navigate to a new page (i.e.{' '}
        <code>event.preventDefault()</code> is called). By capturing the{' '}
        <code>language-toggle</code> event page content can be updated as needed
