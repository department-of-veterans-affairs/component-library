---
title: OMBInfo
name: OMBInfo
---

import OMBInfo from './OMBInfo'

### Code:

```javascript
import OMBInfo from '@department-of-veterans-affairs/component-library/OMBInfo'

<OMBInfo
  resBurden={15}
  ombNumber='Some number'
  expDate='March 15 0000'
/>
```

### Available Props

| Name | Type | Description | Required|
|-|-|-|:-:|
|resBurden| String or Number | Displays the Respondent Burden section in the Privacy Act Statement modal and how many minutes the form is expected to take. | No|
|ombNumber | String | The form's OMB (Office of Management and Budget) control number | No |
| expDate | Date as a string | This should be a formated date, in the format of `MMMM DD YYYY`| Yes |
|children | JSX | Any children nodes will should up in the Privacy Statement Act model | No|

### Rendered Component

<div className="site-c-reactcomp__rendered">
  <OMBInfo
    resBurden={15}
    ombNumber='Some number'
    expDate='March 15 0000'
  />
</div>
